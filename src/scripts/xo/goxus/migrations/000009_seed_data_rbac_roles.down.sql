...
DELETE FROM public.users WHERE email = 'data.worker.nobuenhombre@yandex.ru';
...
WHERE permission_id IN (
    SELECT id FROM public.rbac_permissions
    WHERE slug IN ('data_view', 'data_add', 'data_edit', 'data_delete')
);

-- 6. Delete roles
DELETE FROM public.rbac_roles WHERE slug IN ('data_analytics', 'data_operator');

-- 7. Delete permissions
DELETE FROM public.rbac_permissions WHERE slug IN ('data_view', 'data_add', 'data_edit', 'data_delete');